...
for i in range(0, n_2):  # リストは0から。ラベルは1から。iは0から。
    if data_2[i][4] > 10 and data_2[i][4] < 1500:
        ...
        detectionWhiteRegion3[label_2[1] == i+1, ] = cols[i]
...
for i in range(0, n):  # リストは0から。ラベルは1から。iは0から。
    if data[i][4] > 10 and data[i][4] < 15000:
        # 色を塗る
  ...
  for y in range(480):
    for x in range(640):
      ...
      if labelNumber_2[y][x] == saveCenterWhiteLabels[i]:
        countLabel[i] += 1
        whiteCentertCoordinate.append(np.array([x, y, saveCenterWhiteLabels[i]]))
...
for i in range(len(saveCenterWhiteLabels)):
  ...
  print("ラベル{0}には画素が{1}個あります".format(saveCenterWhiteLabels[i], countLabel[i]))
...
for i in range(len(saveCenterWhiteLabels)):
  ...
  for j in range(len(whiteCentertCoordinate)):
    if whiteCentertCoordinate[j][2] == saveCenterWhiteLabels[i]:
      # 配列のラベル番目に格納する
      whiteCentertCoordinateByLabel[saveCenterWhiteLabels[i]].append(whiteCentertCoordinate[j])
...
print(pycolor.RED+"ラベル4の外形の画素データは{}個です".format(len(outerPointByLabel[4]))+pycolor.END)
...
for i in range(len(saveCenterWhiteLabels)):
  ...
  for j in range(len(whiteCentertCoordinateByLabel[saveCenterWhiteLabels[i]])):
    ...
    accumulationByOnePoints[saveCenterWhiteLabels[i]].append(np.array([accumulationByOnePoint, xc, yc]))
    
  accumulations[saveCenterWhiteLabels[i]].append(accumulation)


for i in range(len(saveCenterWhiteLabels)):
  print(pycolor.GREEN+"ラベル{}:".format(saveCenterWhiteLabels[i])+pycolor.END +  "{}".format(accumulations[saveCenterWhiteLabels[i]][0]))
  print(pycolor.YELLOW+"ラベル{}の最大値: ".format(saveCenterWhiteLabels[i])+pycolor.END +  "{}".format(max(accumulations[saveCenterWhiteLabels[i]][0])))
  print(pycolor.PURPLE+"ラベル{}の最大値が入っているインデックス番号(半径r): ".format(saveCenterWhiteLabels[i])+pycolor.END +  "{}".format(accumulations[saveCenterWhiteLabels[i]][0].index(np.max(accumulations[saveCenterWhiteLabels[i]][0]))))


for i in range(len(saveCenterWhiteLabels)):
  print("ラベル{0}の累積の総和{1}".format(saveCenterWhiteLabels[i], sum(accumulations[saveCenterWhiteLabels[i]][0])))
...
if len(args) < 2: 
  for i in range(len(saveCenterWhiteLabels)):
    ...
    for j in range(len(accumulationByOnePoints[saveCenterWhiteLabels[i]])):
      ...
      maxValue = np.max(accumulationByOnePoints[saveCenterWhiteLabels[i]][j][0])
      r = accumulationByOnePoints[saveCenterWhiteLabels[i]][j][0].index(np.max(accumulationByOnePoints[saveCenterWhiteLabels[i]][j][0]))
...
for i in range(len(saveCenterWhiteLabels)):
  ...
  cv2.circle(img, (xc, yc), radius, (0,0,255), thickness=1, lineType=cv2.LINE_8, shift=0)
